...
public class SellerController {
    ...
    public Result delete(@RequestBody List<Long> ids){
        try {
            int dcount = sellerService.delete(ids);
            if(dcount>0){
    ...
    public Result modify(@RequestBody Seller seller){
        try {
            int mcount = sellerService.modify(seller);
    ...
    public PageInfo<Seller> list(@RequestParam(value = "pageNum",required = false,defaultValue = "1")Integer pageNum,
                                 @RequestParam(value = "size",required = false,defaultValue = "10")Integer size,
                                 @RequestBody Seller seller) {
        return sellerService.getAll(pageNum,size,seller);
    }
